...
from lib.trayicon import TrayIcon
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.generate_dir()
        self.logger = get_logger(f'{get_root_path()}/log/log.txt')
        self.init_theme()
        # 表格相关设置
        ...
        self.db.setDatabaseName(f"{get_root_path()}/task2.db")
        self.db.open()
        self.q = QSqlQuery()
        ...
        self._signal.connect(self.play_scheduler_music)
    ...
    def generate_dir():
        ...
        for dir_name in ['log', 'music', 'backup']:
            full_name = os.path.join(root_path, dir_name)
    ...
    def load_log(self, filename):
        with open(filename) as f:
            ...
            show = []
            for line in txt:
                if line.startswith('mainwindow4.py-'):
                    show.append(line.strip('\n').strip('mainwindow4.py-'))
    ...
    def update_log(self, idx):
        if idx == 2:
            self.load_log(f'{get_root_path()}/log/log.txt')
    ...
    def init_theme(self):
        ...
        self.themeChangeBox.setCurrentIndex(3)
    ...
    def alter_task(self):
        ...
        if self.dialog.exec():
            ...
            res = None if not self.q.next() else self.q.value(0)
    ...
    def load_schedules(self):
        ...
        for record in records:
            ...
            enable = record[5]
            if dow and enable:
                trigger = CronTrigger(hour=h, minute=m, second=s, day_of_week=dow, timezone='Asia/Shanghai')
                self.sch.add_job(self.play_music_by_schedules, trigger=trigger, id=str(record[4]))
    ...
    def play_scheduler_music(self, filename):
        self.play_music(filename)
        self.set_next_run_time()
    ...
    def play_music_by_schedules(self):
        ...
        try:
            ...
            db.setDatabaseName(f"{get_root_path()}/task2.db")
            db.open()
            q = QSqlQuery()
            q.exec("select max(time) from schedules where time < {} and weekdays&{}<>0 and  enable=1".format(
                time_int, 1 << wk))
            while self.q.next():
    ...
    def import_data(self):
        filename, ok = QFileDialog.getOpenFileName(self, "导入文件", f'{get_root_path()}/backup', filter="Ldy Files(*.ldy)")
        if not ok:
            return
    ...
    def export_data(self):
        filename, ok = QFileDialog.getSaveFileName(self, "导出文件", f'{get_root_path()}/backup', filter="Ldy Files(*.ldy)")
        if not ok:
            return
        if filename[-3:] != 'ldy':
            filename += '.ldy'
    ...
    def choose_music(self, flag=0):
        filename, ok = QFileDialog.getOpenFileName(self, "选择音乐", f'{get_root_path()}/music', filter="MusicFile(*.mp3)")
        if ok:
            if not flag:
    ...
    def load_qss(self, qss_file):
        with open(qss_file, "r", encoding='utf-8') as fin:
            qss = fin.read()
